...
const EditListing = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirect back to user's profiles listings list view instead of admin routes
  ...
  if (loading) return <p className="text-center py-20 dir-rtl">در حال بارگذاری...</p>;
    ...
    <div className="min-h-screen bg-white dir-rtl">
      <div className="mx-auto max-w-5xl px-4 py-12">
        ...
        <div className="mb-12">
          ...
          >
            <FaArrowLeft className="scale-x-[-1]" /> بازگشت به اقامتگاه‌ها
          </Link>
          <h1 className="text-4xl font-bold tracking-tight text-neutral-800">ویرایش اطلاعات اقامتگاه</h1>
          <p className="text-neutral-500 mt-3 text-lg">
            مشخصات، ظرفیت‌ها و تصاویر اقامتگاه را به‌روزرسانی کنید.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-12">
          ...
          <Card className="transition-all hover:shadow-xl">
            ...
            <CardContent className="space-y-6">
              {[
                { key: "singleGroups", label: "پذیرش گروه‌های مجردی" },
                  ...
                  >
                    ...
                    <div className="flex gap-3">
                      <Button
                        ...
                        className={`px-8 transition-all ${isAllowed ? "bg-emerald-600 hover:bg-emerald-700 text-white" : ""}`}
                      ...
                      <Button
                        type="button"
                        variant={!isAllowed ? "default" : "outline"}
                        className={`px-8 transition-all ${!isAllowed ? "bg-red-600 hover:bg-red-700 text-white" : ""}`}
                        onClick={() => toggleRule(rule.key as keyof typeof rules)}
                      >
                        مجاز نیست
                      </Button>
          ...
          <Card className="transition-all hover:shadow-xl">
            <CardHeader>
              ...
              <p className="text-neutral-500 text-sm">حداکثر ۱۰ تصویر • تصویر اصلی به عنوان کاور نمایش داده می‌شود</p>
            </CardHeader>
            <CardContent>
              <label className="group flex flex-col items-center justify-center border-2 border-dashed border-blue-300 hover:border-blue-400 bg-blue-50/30 rounded-3xl p-14 cursor-pointer transition-all duration-300">
                <input
                  type="file"
                  multiple
                  accept="image/*"
                ...
                <p className="font-semibold text-lg text-blue-700">کلیک کنید یا تصاویر جدید را اینجا رها کنید</p>
              </label>
          ...
          <Button
            ...
            size="lg"
            disabled={saving}
            className="w-full h-16 text-xl font-bold rounded-3xl bg-blue-700 hover:bg-blue-800 text-white transition-all disabled:bg-blue-400"
          >
            {saving ? "در حال ذخیره تغییرات..." : "ذخیره تغییرات نهایی"}
          </Button>
        </form>
      </div>
      <Footer />
