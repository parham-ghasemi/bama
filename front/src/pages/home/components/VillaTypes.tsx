...
const VillaTypes = () => {
  return (
    <div className="w-full space-y-4">
      <p className="font-bold text-lg md:text-xl">انواع ویلا ها </p>
      <div className="flex overflow-x-auto lg:flex-wrap justify-start md:justify-between gap-4 no-scrollbar">
        {
          types.map((type, index) => (
            <button key={index} className="flex flex-col items-center justify-center flex-shrink-0 w-[80px] h-[80px] md:w-24 md:h-24 cursor-pointer outline outline-gray-200 rounded-lg gap-1.5 focus:outline-2 focus:outline-gray-700">
              <img src={type.icon} alt={type.title} className="w-10 h-10 md:w-12 md:h-12" />
              <span className="text-xs md:text-sm font-bold">{type.title}</span>
            </button>
          ))
        }
      </div>
