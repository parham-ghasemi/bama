  ...
  const handleSearch = () => {
    if (search) {
      const queryParams = new URLSearchParams({
        entry: entryDate.format('jYYYY/jMM/jDD'),
        exit: exitDate.format('jYYYY/jMM/jDD'),
        adults: adults.toString(),
        children: children.toString(),
      });

      navigate(`/villa-results/${encodeURIComponent(search)}?${queryParams.toString()}`);
    }
  };

  return (
    <div ref={headerRef} className="relative z-10 bg-white rounded-2xl md:rounded-[1000px] px-3 md:px-5 py-1 w-full shadow-lg shadow-gray-800/25 flex flex-wrap md:flex-nowrap items-center justify-between hover:bg-gray-200 transition-colors group max-w-6xl">
      <style>{`
      ...
      {options.map((option, idx) => (
        <div
          ...
          className={`flex flex-col gap-1 cursor-pointer min-w-[150px] md:min-w-40 hover:bg-gray-50 hover:rounded-xl hover:outline hover:outline-black py-2 md:py-4 px-2 transition-all duration-300 relative flex-1 md:flex-none ${option.hiddenClass}`}
          key={`header-option-${idx}`}
        >
          <p className="font-semibold text-xs md:text-sm">{option.title}</p>
          {idx === 0 ? (
            <input
              type="text"
              value={search ?? ''}
              ...
              className="text-xs md:text-sm text-gray-800 font-light bg-transparent border-none outline-none"
            />
          ) : idx === 1 ? (
            <div onClick={(e) => e.stopPropagation()}>
              <DatePicker
                ...
                numberOfMonths={1} // Reduce to 1 month on mobile
                showOtherDays={true}
                value={entryDate}
                ...
                render={(value, openCalendar) => (
                  <p onClick={openCalendar} className="text-xs md:text-sm text-gray-800 font-light">
            ...
            <p className="text-xs md:text-sm text-gray-800 font-light">
      ...
      <div onClick={handleSearch} className="h-12 w-12 bg-[#2891C3] group-hover:brightness-110 hover:brightness-125 rounded-xl md:rounded-full hover:scale-105 transition cursor-pointer flex items-center justify-center mx-auto md:mx-0 ">
