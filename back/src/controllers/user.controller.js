...
exports.getProfile = async (req, res) => {
  try {
    const user = await User.findById(req.user.id).select('-otp -otpExpiration');
    if (!user) return res.status(404).json({ msg: 'User not found' });
    res.json(user);
  } catch (err) {
    res.status(500).json({ msg: err.message });
...
exports.deposit = async (req, res) => {
  try {
    ...
    res.json({ msg: 'Deposited successfully', balance: user.walletBalance });
  } catch (err) {
    res.status(500).json({ msg: err.message });
  }
};

exports.getLikedVillas = async (req, res) => {
  try {
    const user = await User.findById(req.user.id).populate('LikedVillas');
    if (!user) return res.status(404).json({ msg: 'User not found' });
    res.json(user.LikedVillas);
  } catch (err) {
    res.status(500).json({ msg: err.message });
  }
};
...
exports.getPastReservations = async (req, res) => {
  try {
    ...
    const pastReservations = user.history.filter(reservation => reservation.until < current);
...
exports.getUpcomingReservations = async (req, res) => {
  try {
    ...
    const current = moment().format('jYYYY/jMM/jDD');
    const upcomingReservations = user.history.filter(reservation => reservation.until >= current);
